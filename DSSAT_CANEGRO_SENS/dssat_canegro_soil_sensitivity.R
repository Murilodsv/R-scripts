#--- Called from main
#--- Create multiple DSSAT soil profiles and save to a .SOL
#--- Murilo Vianna
#--- May-2018
...
#------ The baseline should also be included in the list
scen_raw = read.csv("scenarios.csv")
scen = scen_raw[scen_raw$scenario!=0,]
scen = rbind(scen,data.frame(scenario = 0, 
                      var      = (scen_raw$var[scen_raw$scenario==0][1]), 
                      id       = (scen_raw$id[scen_raw$scenario==0][1]) ,
                      val      = (scen_raw$val[scen_raw$scenario==0][1]),
                      pos      = (scen_raw$pos[scen_raw$scenario==0][1])))

len  = length(scen$var)  # number of scenarios + baseline

#--- list of single scenarios
l_id = unique(scen$id)

#--- id for files sufixname
fsufix = simID

#--- reading master soil profile
#--- Must be a .SOL file with the master profile (baseline and the mirror profile, see Template.sol)
sol     = readLines("Template.sol")

#--- Output file.sol
outsol = paste("C:/",dv,"/Soil/",sname,sep="")
...
#--- Start of scenarios profiles (n = len)
for(sc in l_id){
  
  new_prof = scen[scen$id == sc,]
  
  print(paste("Creating soil profile ",sc,sep=""),quote = F)
  
  s  = unique(new_prof$scenario)
  v  = unique(new_prof$var)
  id = unique(new_prof$id)
  
  if(length(new_prof$scenario)==1){
    vl = unique(new_prof$val)
    ps = unique(new_prof$pos)
  }else{
    vl = new_prof$val
    ps = new_prof$pos
  }
  ...
  new_sol = gsub(c("ID"),id,msol_ID)
  new_sol = gsub(c("SCEN"),paste(s,v,sep="_"),new_sol)
  ...
  if(length(new_prof$scenario)==1 & ps[1] == "pd"){nsol_pd[,as.character(v)] = vl}
  ...
  for(sl in 1:nlayers){
    ...
    if(length(ps) > 1 & substr(ps[sl],1,2) == "ld"){
      nsol_ld[,as.character(v)] = vl[ps == paste("ld",sl,sep="")]
    }
